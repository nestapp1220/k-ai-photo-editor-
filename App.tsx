...
const App: React.FC = () => {
  ...
  const handleApplyFilter = useCallback(async (filterPrompt: string) => {
    if (!currentImage) {
      setError(t('errorNoImageApplyFilter'));
      return;
    }
    ...
    try {
        ...
        const errorMessage = err instanceof Error ? err.message : t('errorUnknown');
        setError(`${t('errorFilterFailed')} ${errorMessage}`);
        console.error(err);
    } finally {
        setIsLoading(false);
    }
  ...
  const handleApplyAdjustment = useCallback(async (adjustmentPrompt: string) => {
    if (!currentImage) {
      setError(t('errorNoImageApplyAdjustment'));
      return;
    }
    ...
    try {
        ...
        const errorMessage = err instanceof Error ? err.message : t('errorUnknown');
        setError(`${t('errorAdjustmentFailed')} ${errorMessage}`);
        console.error(err);
    } finally {
        setIsLoading(false);
    }
  ...
  const handleApplyCrop = useCallback(() => {
    ...
    if (!ctx) {
        setError(t('errorCropProcessFailed'));
        return;
    }
      ...
      <div className="w-full max-w-6xl mx-auto flex flex-col items-center gap-6 animate-fade-in">
        <div className="relative w-full shadow-2xl rounded-xl overflow-hidden bg-black/20">
            ...
            {displayHotspot && !isLoading && activeTab === 'retouch' && (
                <div 
                    className="absolute rounded-full w-6 h-6 bg-green-500/50 border-2 border-white pointer-events-none -translate-x-1/2 -translate-y-1/2 z-10"
        ...
        <div className="w-full bg-gray-900/60 border border-gray-700/80 rounded-lg p-2 flex items-center justify-center gap-2 backdrop-blur-lg">
            {(['retouch', 'adjust', 'filters', 'crop'] as Tab[]).map(tab => (
                 <button
                    key={tab}
                    onClick={() => setActiveTab(tab)}
                    className={`w-full capitalize font-semibold py-3 px-5 rounded-md transition-all duration-200 text-base relative ${
                        activeTab === tab 
                        ? 'text-teal-300' 
                ...
                >
                    {t(tab)}
                    {activeTab === tab && <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-1/2 h-1 bg-gradient-to-r from-green-400 to-teal-400 rounded-full"></div>}
                </button>
            ))}
        </div>
            ...
            {activeTab === 'retouch' && (
                <div className="flex flex-col items-center gap-4">
                    <p className="text-md text-gray-400">
                        {editHotspot ? t('promptDescription') : t('promptClick')}
                    </p>
                    <form onSubmit={(e) => { e.preventDefault(); handleGenerate(); }} className="w-full flex items-center gap-2">
                        <input
                            ...
                            placeholder={editHotspot ? t('promptExample') : t('promptPlaceholder')}
                            className="flex-grow bg-gray-800 border border-gray-700 text-gray-200 rounded-lg p-5 text-lg focus:ring-2 focus:ring-green-500 focus:outline-none transition w-full disabled:cursor-not-allowed disabled:opacity-60"
                            disabled={isLoading || !editHotspot}
                        />
                        <button 
                            type="submit"
                            className="bg-gradient-to-br from-green-600 to-teal-500 text-white font-bold py-5 px-8 text-lg rounded-lg transition-all duration-300 ease-in-out shadow-lg shadow-teal-500/20 hover:shadow-xl hover:shadow-teal-500/40 hover:-translate-y-px active:scale-95 active:shadow-inner disabled:from-gray-700 disabled:to-gray-600 disabled:shadow-none disabled:cursor-not-allowed disabled:transform-none"
        ...
        <div className="flex flex-wrap items-center justify-center gap-3 mt-6">
            ...
            <button 
                ...
                className="flex-grow sm:flex-grow-0 ml-auto bg-gradient-to-br from-green-600 to-teal-500 text-white font-bold py-3 px-5 rounded-md transition-all duration-300 ease-in-out shadow-lg shadow-teal-500/20 hover:shadow-xl hover:shadow-teal-500/40 hover:-translate-y-px active:scale-95 active:shadow-inner text-base"
            >
                {t('downloadImage')}
            </button>
        </div>
